...
for i in $(seq 1 $max_retries); do
    ...
    if [ $i -eq $max_retries ]; then
        ...
        echo "   Check logs with: docker compose -f docker/docker-compose.yml logs db"
        exit 1
    fi
...
if ! docker compose -f docker/docker-compose.yml exec -T api alembic upgrade head; then
    echo "❌ Migration failed. Check logs with: docker compose -f docker/docker-compose.yml logs api"
    exit 1
fi
...
if ! docker compose -f docker/docker-compose.yml exec -T -e ENV=development api python apps/api/scripts/reset_dev_data.py; then
    echo "❌ Data reset failed. Check logs with: docker compose -f docker/docker-compose.yml logs api"
    exit 1
fi
...
echo "   docker compose -f docker/docker-compose.yml exec api alembic current"
echo ""
echo "   Check organizations:"
echo "   docker compose -f docker/docker-compose.yml exec api psql -U consentvault -d consentvault -c \"SELECT * FROM organizations;\""
echo ""
echo "   Check users:"
echo "   docker compose -f docker/docker-compose.yml exec api psql -U consentvault -d consentvault -c \"SELECT id, email, role, active FROM users;\""
echo ""
echo "   Check API keys:"
echo "   docker compose -f docker/docker-compose.yml exec api psql -U consentvault -d consentvault -c \"SELECT id, name, organization_id, active FROM api_keys;\""
echo ""
